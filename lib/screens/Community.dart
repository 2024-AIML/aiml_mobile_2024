...
class _CommunityState extends State<Community> {
        ...
        setState(() {
          posts = data.map((post) {
            return {
              ...
              'latitude': double.tryParse(post['latitude'].toString()) ?? 0.0,
              'longitude': double.tryParse(post['longitude'].toString()) ?? 0.0,
            };
          }).toList();

          for (var post in posts){
            _addMarker(post['latitude'], post['longitude'], post['title'],);
          }
        });
      } else {
  ...
  void _addMarker(double latitude, double longitude, String title) {
    final marker = NMarker(
      id: 'marker_${latitude}_${longitude}', // 고유 ID
              ...
              child: NaverMap(
                options: NaverMapViewOptions(
                  initialCameraPosition: NCameraPosition(
                    target: NLatLng(posts[0]['latitude'], posts[0]['longitude']),
                    zoom: 14,
                  ),
                ),
